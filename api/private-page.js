...
function commonHeaders(res,mode,role='unknown'){
  ...
  res.setHeader('X-Bizu-Private-Proxy','v13-admin-security-v3');
...
export default async function handler(req,res){
  ...
  try{
    ...
    commonHeaders(res,mode,role);
    res.setHeader('X-Bizu-UI-Patch',mode==='admin'?'admin-management-v2+notices-stability+security-v3':'shared-core-v1+questions-v2+quick-test-v2');
    if(cookies.length)res.setHeader('Set-Cookie',cookies);
    return res.end(body);
  }catch(err){
    res.statusCode=503;
